...
import { glossaryTermsInText, glossaryFingerprintForText, loadGlossary } from '../lib/glossary.js';
  ...
  (async () => {
    ...
    try {
      if (msg.type === 'openViewer') {
        const tab = await chrome.tabs.create({ url: viewerUrl(msg.file, msg.reopen) });
...
async function handleTranslate(port, msg, requests) {
  ...
  try {
    ...
    // 术语指纹参与译文缓存身份（见 translation-cache.js）：锁定/修改命中本页
    // 原文的术语后，旧缓存自然失效重译。公式 OCR 与语言无关，不参与。
    cfg.glossaryFingerprint = formulaJob ? '' : glossaryFingerprintForText(glossary, text);
  } catch { cfg.glossary = []; cfg.glossaryFingerprint = ''; }
...
function viewerUrl(file, reopen) {
  const base = chrome.runtime.getURL('src/viewer/viewer.html');
  if (file) return `${base}?file=${encodeURIComponent(file)}`;
  if (reopen) return `${base}?reopen=${encodeURIComponent(reopen)}`;
  return base;
}
