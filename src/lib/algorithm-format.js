...
// (iOS Safari < 16.4 has no lookbehind, so the boundary is a consuming
// group: match[1] = boundary text, match[2] = step number.)
const STEP_MARK_CORE = String.raw`(\d{1,3})\s*(?:[:：](?!${MATH_RANGE_AFTER_COLON})\s*|[.．]\s+)`;
const STEP_MARK = String.raw`(^|\D)${STEP_MARK_CORE}`;
const STEP_MARK_RE = new RegExp(STEP_MARK, 'g');
// Line-anchored: "^\s*" already guarantees no digit precedes the number.
const STEP_AT_LINE_RE = new RegExp(String.raw`^\s*${STEP_MARK_CORE}`);
...
export function reflowCompactAlgorithmLines(text) {
  ...
  for (let i = 0; i < matches.length; i += 1) {
    ...
    // Next marker's consumed boundary char (m[1]) still belongs to this body.
    const end = i + 1 < matches.length ? matches[i + 1].index + matches[i + 1][1].length : flat.length;
...
export function splitAlgorithmTitleAndBody(text) {
  ...
  // Marker really starts after the consumed boundary char (m[1]).
  const markerIndex = m ? m.index + m[1].length : -1;
  if (!m || markerIndex <= 0) {
    return { title: '', body: source };
  }
  const before = source.slice(0, markerIndex).trim();
  const body = source.slice(markerIndex).trim();
