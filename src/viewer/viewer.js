...
async function translatePageVision(p) {
  ...
  const preservedDraft = qualityRetry ? String(p.translationText || '').trim() : '';
...
function ensureSelectionPopover() {
  ...
  lockBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      showToast(`已锁定术语：${term.slice(0, 24)} → ${translation.slice(0, 24)}`);
    } catch (error) {
...
function bindUi() {
  ...
  els.btnExportDoc?.addEventListener('click', () => { void exportDocumentTranslation(); });
