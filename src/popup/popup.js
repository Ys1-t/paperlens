...
function renderRecentReadings() {
  ...
  for (const entry of items) {
    ...
    btn.title = entry.sourceUrl
      ...
      : '本地文件：一键重开（首次可能需确认文件访问权限）';
    btn.addEventListener('click', () => (
      entry.sourceUrl ? openViewer(entry.sourceUrl) : openViewerReopen(entry.docKey)
    ));
    if (!entry.sourceUrl) {
...
/** 最近阅读的本地文件：带 docKey 打开阅读器，viewer 端用已存句柄一键重开。 */
function openViewerReopen(docKey) {
  chrome.runtime.sendMessage({ type: 'openViewer', reopen: docKey }, (response) => {
    handleOpenViewerResponse({
      response,
      lastError: chrome.runtime.lastError,
      close: () => window.close(),
