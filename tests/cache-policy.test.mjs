...
test('glossary fingerprint namespaces the cache identity without breaking legacy entries', () => {
  const config = {
    protocol: 'openai',
    baseUrl: 'https://api.example/v1',
    model: 'model-x',
  ...
  const legacy = buildTranslationCacheIdentity(config, 'IMG:page5');
  // 空指纹（未锁术语 / 本页无命中）→ 与历史身份逐字节一致，存量缓存不作废。
  assert.equal(buildTranslationCacheIdentity({ ...config, glossaryFingerprint: '' }, 'IMG:page5'), legacy);
  // 命中术语 → 身份变化 → 旧缓存自然绕过；不同译法互不串缓存。
  const locked = buildTranslationCacheIdentity(
    { ...config, glossaryFingerprint: 'learning to optimize=>学习优化' }, 'IMG:page5',
  );
  assert.notEqual(locked, legacy);
  assert.notEqual(
    buildTranslationCacheIdentity(
      { ...config, glossaryFingerprint: 'learning to optimize=>优化学习' }, 'IMG:page5',
    ),
    locked,
  );
});
